...
namespace ConsoleApp
{
    public class SalesPerson
    {
        ...
        public void DisplaySaleAttempt(bool success, string itemName)
        {
            ...
            if (success)
            {
                Console.WriteLine(string.Format("Thread {0}: {1} sold {2}", threadId, Name, itemName));
            }
            else
            {
                Console.WriteLine(string.Format("Thread {0}: {1}: Out of stock of {2}", threadId, Name, itemName));
            }
        }
    }
}